...
export const Applicants = () => {
  ...
  const [currentFilter, setCurrentFilter] = useState<ApplicantFilter>(ApplicantFilter.All);
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [applicants, setApplicants] = useState<IApplicant[] | null>(null);
  const [filteredApplicants, setFilteredApplicants] = useState<IApplicant[] | null>(null);
  ...
  useEffect(() => {
    ...
    if (currentFilter === ApplicantFilter.All) {
      setFilteredApplicants(applicants);
      setTotalPages(Math.ceil((applicants?.length || 0) / perPage));
    }

    if (currentFilter === ApplicantFilter.Applied) {
      const newApplicants = applicants?.filter(a => a.status === JobApplyStatus.Applied) || [];
      setFilteredApplicants(newApplicants);
      setTotalPages(Math.ceil((newApplicants.length) / perPage));
    }

    if (currentFilter === ApplicantFilter.Accepted) {
      const newApplicants = applicants?.filter(a => a.status  === JobApplyStatus.Accepted) || [];
      setFilteredApplicants(newApplicants);
      setTotalPages(Math.ceil((newApplicants.length) / perPage));
    }

    if (currentFilter === ApplicantFilter.Rejected) {
      const newApplicants = applicants?.filter(a => a.status  === JobApplyStatus.Rejected) || [];
      setFilteredApplicants(newApplicants);
      setTotalPages(Math.ceil((newApplicants.length) / perPage));
    }
    ...
    setCurrentPage(1);
    setPages([1, 2, 3]);
  }, [currentFilter, applicants, perPage]);
  ...
  }, [currentPage, pages, filteredApplicants, perPage]);

  const fetchApplicants = useCallback(async () => {
    try {
      setIsLoading(true);
      const resp: AxiosResponse<any, any> = await getApplicants();
      const applicantsFetched: IApplicant[] = resp.data.data.data;
      console.log(applicantsFetched);
  ...
  useEffect(() => {
    fetchApplicants();
    setCandidateHeaderMessage(CandidateHeaderMessages.Applicants);
        ...
        <div className="container search-workers__workers search-workers__workers--applicants">
          <div className="container workers">
            <div className="workers__list">
              {filteredApplicants?.slice(perPage * (currentPage - 1), perPage * currentPage).map(applicant => {
                const {
                  applicant_id,
                  status,
                  Candidate,
            ...
            <div className="workers__bottom">
              <div className="pagination">
                <div className="pagination__left">
                  ...
                  <div className="pagination__text pagination__text--bold">{filteredApplicants?.length}</div>
