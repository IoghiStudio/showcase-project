...
import { IApplyJob, applyJob, getFavoriteJobs, getJobs, getOneJob, makeJobFavorite, makeJobUnfavorite } from '@/services/api/job.service';
...
export const FavoriteJobs = () => {
  ...
  const fetchJobs = useCallback(async (params?: string) => {
    try {
      setIsLoading(true);
      const resp: AxiosResponse<any, any> = await getFavoriteJobs(params || '');
  ...
  const fetchJobsWithFilters = useCallback((pageNumber: number, status?: JobApplyStatus) => {
    let params: string = '';
    const paramList = [];

    if (country) {
      paramList.push(`country_id=${country.country_id}`);
    }
    ...
    if (workplaceType) {
      paramList.push(`workplace_type=${workplaceType}`);
    }
    ...
    if (params.length) {
      params += `&page=${pageNumber}&pageSize=${perPage}`;
    } else {
      params += `?page=${pageNumber}&pageSize=${perPage}`;
    }
